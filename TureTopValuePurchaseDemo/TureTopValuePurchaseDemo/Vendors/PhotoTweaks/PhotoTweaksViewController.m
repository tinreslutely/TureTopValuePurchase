...
- (instancetype)initWithImage:(UIImage *)image touches:(BOOL)touches cropSize:(CGSize)cropSize
{
    if (self = [super init]) {
        _image = image;
...
- (void)setupSubviews
{
    self.photoView = [[PhotoTweakView alloc] initWithFrame:self.view.bounds image:self.image cropSize:_cropSize touches:_isTouches];
    ...
    [cropBtn setTitle:NSLocalizedStringFromTable(@"截图", @"PhotoTweaks", nil) forState:UIControlStateNormal];
